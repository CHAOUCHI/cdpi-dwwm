...
## Allez plus loin : Variable d'envrionnement, DNS et hostname dans Docker Compose

Il y a un soucis de conception dans notre application. Pour se connecter à la base de données le développeur à inscrit le hostname du container (`bdd`).
...
**Le problème c'est que si le HOSTNAME de la bdd change, il faudra REMODIFIER LE CODE** ce qui n'est pas le travail du DevOps mais du développeur, c'est un problème de conception de l'application.

On veut que le DevOps puisse définir le hostname de la bdd de l'exterieur de l'application, idéaliement dans le compose.yaml; pour ce faire il nous FAUT une ***variable d'envrionnement***.


### Modifier le code pour utiliser une variable d'environnement

En PHP, on peut utiliser la fonction `getenv` pour récupérer la valeur d'une variable d'environnement, et on peut aussi définir une valeur par défaut au cas où la variable d'environnement n'est pas définie.

> Nodejs : `process.env.VARIABLE_NAME || "default_value"`
> PHP : `getenv("VARIABLE_NAME") ?: "default_value"`
> Python : `os.getenv("VARIABLE_NAME", "default_value")`
> Java : `System.getenv("VARIABLE_NAME") != null ? System.getenv("VARIABLE_NAME") : "default_value"`

1. Modifiez le code de `index.php` pour utiliser une variable d'environnement pour le hostname de la base de données et les autres paramètres de connexion à la base de données (pour que le DevOps puisse les définir dans le compose.yaml).
...
2. Modifier compose.yaml pour que docker déclare ces variables d'envrionnement lors du lancement des conteneurs, pour cela on utilise la commande `environment` dans le service web.
...
services:
  web:
    ...
    ports:
      - "8080:80"
    # Déclaration des variables d'environnement pour que le service web puisse se connecter à la base de données en utilisant les bonnes informations de connexion
    environment:
      DATABASE_HOST: bdd
      DATABASE_NAME: ma-bdd
      DATABASE_USER: root
...
Avec cette configuration, le service web va pouvoir se connecter à la base de données en utilisant les variables d'environnement définies dans le compose.yaml, et si jamais on change le hostname de la base de données dans le compose.yaml, il n'y aura pas besoin de modifier le code de l'application, ce qui est une bonne pratique de conception d'application.

> *Note* : Utilisez cette pratique pour toutes variables de configuration secretes ou susceptibles de changer en fonction de l'environnement (dev, staging, prod) pour éviter d'avoir à modifier le code de l'application à chaque changement de configuration : apikey, hostname de la base de données, username et mot de passe de la base de données, etc.

> *Note Securité* : Dans une pipeline CICD gitlab ou GitHubActions il est possible de définir des variables d'environnement dans les paramètres du projet, et ces variables d'environnement seront automatiquement injectées dans les conteneurs lors du lancement de l'application, ce qui permet de ne pas stocker les secrets dans le code source ou dans le compose.yaml.


## Allez plus loin : Les volumes Docker pour persister les données de la base de données

Si le container est supprimé les données de la base de données seront perdues, pour éviter cela il faut utiliser les volumes Docker pour persister les données de la base de données sur le disque dur de l'hôte.

Il faut modifier le fichier compose.yaml pour ça :

1. Déclarer le volume dans la section `volumes` du compose.yaml
