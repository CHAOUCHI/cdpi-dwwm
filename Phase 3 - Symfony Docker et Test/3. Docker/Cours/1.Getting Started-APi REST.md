...
## Conteneurisation d'une API REST nodejs

Une api rest est une application qui expose des endpoints (routes) pour permettre à d'autres applications de communiquer avec elle, souvent via le protocole HTTP. Dans notre cas, on va créer une API REST simple avec Node.js et Express qui écoute sur le port 3000 et qui retourne un message JSON lorsqu'on accède à la route racine (*http://localhost:3000*).
