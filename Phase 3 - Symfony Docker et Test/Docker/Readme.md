...
Chaque application était codée pour l'OS du serveur (Windows ou Linux) et les dépendances de l'application devaient être installées manuellement sur le serveur.
...
Cette solution entraîne donc des soucis :
...
- Difficulté de résolution de problèmes (Plantage de l'application affectant le serveur entier)

### VM (Virtual Machine)

On a donc par la suite inventé les machines virtuelles (VM) pour résoudre ces problèmes.
...
En 2010, Solomon Hykes, un ingénieur franco-américain, a créé Docker, une solution pour simplifier le déploiement et le développement d'applications en s'inspirant du concept de *conteneur de fret*.
