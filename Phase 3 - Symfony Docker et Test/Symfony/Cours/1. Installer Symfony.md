...
Documentation officiel : https://symfony.com/doc/current/setup.html
...
2. Vérifier les dependences neccessaires 
```
...
```bash
sudo apt update
# Installe les paquets de ondrej qui permet de changer de version de php si besoin(Symfony neccessite au moins la 8.4)
sudo apt install software-properties-common
sudo add-apt-repository ppa:ondrej/php
sudo apt update

# Installe php 8.4 et les extensions neccessaires à symfony (voir la doc)
...
4. Faite la commande `php -v` pour vérifier que vous avez la bonne version.
```bash
php -v
```
...
6. Faite la commande `composer` pour vérifier si il est bien installé et disponible dans la console.
```bash
composer
``` 
Vous devez voir ce résultat dans la console :
...
7. Dans un nouveau terminal executé la commande `symfony check:requirements` le résultat suivant signifie que tout va bien :
```bash
symfony check:requirements
``` 
Vous devez voir ce résultat dans la console :
...
```
8. Pour finir installer `sqlite3` (un SGDB plus léger que MySQL en local il n'a pas besoin de logins ni du port 3306 TCP) et `sqlitewebbrowser` (le phpmyadmin de sqlite)
```bash
...
## Hello World Symfony
> SI CE N'EST PAS FAIT, verifier que vous avez bien décilé votre identité à git
> ```bash
