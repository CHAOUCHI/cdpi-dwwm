...
*Apache est l'un des serveurs HTTP les plus utilisés au monde.*
...
*Plusieurs clients HTTP demandent une ressource à un serveur web (Apache).*
...
2.  **Déploiement du site web**
    ...
    ```html
    <h1>Bonjour, je suis un titre écrit en HTML. Copiez-moi dans le fichier index.html !</h1>
    ```
<!-- 
#### **Configurer Apache2**
...
##### **1. Changer le dossier racine du site web**
   ...
   - On peut utiliser la commande chmod mais le plus simple est de l'ouvrir avec l'éditeur de texte nano.
   ```bash
...
### Exercice 1 :
1. Créez un fichier HTML basique avec une structure correcte et ajoutez un titre à votre page dans la balise `<h1>`.
...
### Exercice 1 :
Créez trois paragraphes de texte dans le `body` sans utiliser la balise `<p>`. Observez comment les paragraphes ne passent pas à la ligne.

### Exercice 2 :
Ajoutez des balises `<p> </p>` autour de chaque paragraphe de texte et comparez la différence avec du texte brut sans balises.
...
### Exercice 1 :
Créez un titre principal en utilisant la balise `<h1>` et plusieurs sous-titres avec `<h2>` et `<h3>`.

### Exercice 2 :
Changez l'ordre des balises de titre et regardez comment cela affecte la taille du texte.
...
- **`src`** : L'URL ou le chemin vers l'image.
- **`alt`** : Un texte alternatif qui s'affiche si l'image ne peut pas être chargée.

Lorsque le navigateur lit l'attribut `src` de la balise `img`, il effectue une requête HTTP au serveur web Apache.

Il faut donc ajouter l'image `image.jpg` dans le dossier `DocumentRoot` de notre serveur web.

1. Ajoutez une image dans le dossier `DocumentRoot` du serveur : par défaut `/var/www/html`.

### Exercice 1 :
Ajoutez une image dans votre page HTML en utilisant l'attribut `src` pour spécifier son emplacement sur le serveur.

### Exercice 2 :
Essayez d'insérer une image avec un chemin incorrect et observez comment le texte alternatif défini dans `alt` est affiché.

### Exercice 3 :

Décrivez le processus de requêtes échangées entre le client et le serveur depuis le chargement de la page à partir de ce code source HTML.
```html
<!DOCTYPE html>
<html>
<head>
...
### Exercice 1 :
Créez un lien cliquable avec la balise `<a>`.

### Exercice 2 :
Ajoutez plusieurs liens vers des sites différents.
...
### Exercice 1 :
Utilisez la balise `<br>` pour séparer des phrases sur différentes lignes sans utiliser la balise `<p>`.

### Exercice 2 :
Ajoutez une ligne horizontale à la fin de votre page avec la balise `<hr>`.
...
2. Ajoutez la balise `link` dans le `head` pour inclure le fichier `style.css` à la page.
```html
<!DOCTYPE html>
<html>
<head>
...
Si je veux mettre un `background-color` sur la balise `p` et la balise `body` complète, je peux :
```css
p {
    color: red;
}
...
> Retrouvez toute la documentation HTML et CSS sur w3schools.

### Exercice 1 :
Appliquez une couleur de texte rouge à un paragraphe et une couleur de fond bleu clair à un `div`.

### Exercice 2 :
Essayez d'utiliser des couleurs en utilisant les noms (`blue`, `red`), les valeurs hexadécimales (`#ff0000`) et les valeurs RGB (`rgb(255, 0, 0)`).
...
### Exercice 1 :
Ajoutez une bordure noire solide de 3 pixels autour d'un `div`.

### Exercice 2 :
Essayez différents styles de bordures comme `dashed` ou `dotted`.
...
### Exercice 1 :
Appliquez une marge de 20 pixels autour d'un `div` et observez comment il se déplace.

### Exercice 2 :
Essayez de spécifier des marges différentes pour chaque côté du `div` (par exemple `margin: 10px 20px 30px 40px`).
...
### Exercice 1 :
Ajoutez un `padding` de 20px à un `div` et observez comment l'espace entre le contenu et la bordure change.

### Exercice 2 :
Essayez d'ajouter différents `padding` sur chaque côté (par exemple, `padding: 10px 15px 20px 25px`).
...
### Exercice 1 :
Ajoutez une ombre noire douce à un `div` en utilisant `box-shadow`.

### Exercice 2 :
Modifiez l'intensité du flou et de la couleur de l'ombre pour voir comment cela affecte l'apparence du `div`.
...
### Exercice 1 :
Créez une "carte" en utilisant un `div` avec du `padding`, une `box-shadow`, et un fond de couleur.

### Exercice 2 :
Ajoutez plusieurs cartes avec des marges entre elles.
...
### Exercice 1 :
Créez un conteneur flex avec trois `div` à l'intérieur et alignez-les horizontalement au centre.

### Exercice 2 :
Essayez de changer la direction du `flex` en utilisant `flex-direction: column` pour organiser les éléments verticalement.
...
### Exercice 1 :
Créez une grille avec quatre éléments, organisés en deux colonnes.

### Exercice 2 :
Changez la taille des colonnes en utilisant des unités telles que `fr` (fraction) ou des pixels.
...
# 2. Petit Projet :
1. Créez un petit site en HTML et CSS.
2. Mettez-le en production sur votre serveur web.
3. Vérifiez qu'il soit disponible sur tous les PC du LAN de l'école. *(Attention au mode pont de la VM serveur)*.
