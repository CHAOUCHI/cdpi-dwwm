...
2. Faites un `console.log(pokemons)` pour vous informer des attributs de chaque pokémon du tableau de pokémons.
...
#### Code : 
```js
fetch(url)          // Remplacez l'url par l'url de la requête HTTP à exécuter
...
.then(function(data) {    // la 2nde fonction callback fournit les données   
    console.log(data);
