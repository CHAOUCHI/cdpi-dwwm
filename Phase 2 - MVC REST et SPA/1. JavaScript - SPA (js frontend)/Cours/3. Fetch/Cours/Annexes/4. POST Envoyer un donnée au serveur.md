...
## 2. POST Envoyer des donnée en serveur

Pour envoyer des données aux  serveur je peut simplement :
1. Créer une formulaire `POST`
2. Récupérer le forumlaire avec `querySelector`
3. Effectuer le fetch lors de l'event `submit` du formulaire
4. Passer les formData du formulaire dans le deuxième paramètre de `fetch()` pour remplir `$_POST` coté PHP.
...
```js
const form_elem = document.querySelector(".user_form");

form_elem.onsubmit = onSubmit;
