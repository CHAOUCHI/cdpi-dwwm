...
Acteurs : Utilisateur non connecté, Utilisateur connecté, Admin (même UX mais droits supérieurs).
...
- Pages/Routes :
  ...
  - /create-post (ou modal)
- CRUD réduit :
  - Create post, Create comment, Delete post/comment (avec vérif d’autorisation côté API).
- Gestion des erreurs visibles (toast / alert) pour cas critiques : mauvais identifiants, actions non autorisées, données invalides, ressources absentes.
- Loading states et placeholders pour requêtes asynchrones.
...
Critères d'acceptation (par cas critique fourni)
...
- API client
  - Types/Interfaces: User, Post, Comment, ApiError.
  - Générer types si possible à partir du back (OpenAPI) en bonus.