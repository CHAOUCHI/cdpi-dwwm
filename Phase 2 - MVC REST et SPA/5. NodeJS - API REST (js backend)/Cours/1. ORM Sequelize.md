...
## Sequelize ORM - Faire du SQL sans ecrire de SQL
...
3. Installez les extensions npm nécessaires au projet, à savoir :
...
```
Sequelize s'occupera donc de la redaction de CREATE TABLE et du placement des clés étrangères.
...
>Avec cette api nous pourrons concevoir une application front-end séparé du back-end à la manière du projet Pokedex
...
2. Utiliser la méthode `sequlize.define` de l'instance `sequelize` et définir le nom et les champs de la nouvelle table.
```js
import { Sequelize, DataTypes } from "sequelize";
...
> force : true vide les données précedentes.
...
> Lien du cours officel sur la doc de sequelize : https://sequelize.org/docs/v6/core-concepts/model-querying-basics/#simple-select-queries
...
2. Effectuez un autre `User.destroy()` qui cette fois supprime billy **EN FONCTION de son username**
...
```
> Lien du cours officel sur la doc de sequelize :
> findByPk : https://sequelize.org/docs/v6/core-concepts/model-querying-finders/#findbypk
> Il existe de nombreuse fonctions `finders` regarder les autres fonctions dans la doc.
...
Pour modifier le champs d'une table,par exemple le mot de passe d'un utilisateur, je peux utiliser la fonction Model.update(), elle fonctionne d'une façon très similaire au findAll.
...
1. Un objet qui contient les valeurs à mettre à jours en tant qu'attributs.
2. Un autre objet qui contient un champs `where` avec les conditions de cherche de la lignes à modifier.

> Lien du cours officel sur la doc de sequelize : https://sequelize.org/docs/v6/core-concepts/model-querying-basics/#simple-update-queries
...
Dans second parti nous allons compèter notre BDD d'un second modèle et effectuer une ralation OneToMany.

Vous verrez comment sequelize simplifie la gestion des clé etrangère et de INNER JOIN.

### 1. Créer TaskModel

Pour rappel les tables ressemble à ceci :
```mermaid
erDiagram
    USER {
        int id PK
    ...
    USER ||--o{ Task : has
```
Je défini donc le TaskModel de cette façon :
...
Et la vous vous dite surement :

**"Mais, tu as oublié les champs `id`,`createdAt` et `userId` dans le modèle !"**

"*Non* je ne les ai pas oubliés."

Par défaut sequelize crée automatiquements les champs :
- id pour la clé primaire
- createdAt et updatedAt

**D'accord mais quand est t-il de la clé étrangère userId ?!**

### 2. Association OneToMany TaskModel et UserModel

L'utilisateur peut crée une tache qui doit lui être associé. Il nous faut donc la clé étrangère du UserModel dans un colonne du TaskModel.
...
// 3. Mise en place dela relation OneToMany

const User = sequelize.models.User;
const Task = sequelize.models.Task;

User.hasMany(Task);   // liaison User has Many Task
Task.belongsTo(User); // lisaison Task belong to One User
// Ensemble ces deux lisaison formes donc une relation OneToMany

await sequelize.sync();
```

Grâce aux fonctions hasMany et belongsTo sequelize va se charger de placer la clé etrangère userId dans la table de Tache.

> ***Astuce*** : c'est toujours le `Model` situé à gauche de la fonction belongs qui reçoit la cé étrangère.
...
- Une table `Task` avec ses champs y compris le champs `userId` créer par les fonctions `hasMany` et `belongsTo`

### 3. CRUD TaskModel
Dans un *OneToMany* je souhaite accéder aux données d'un *Model* à partir d'un autre :
- *CREATE* : Une tache et l'associer à un utilisateur
- *READ* : Toutes les taches d'un utilisateur à partir du modèle `User`
- *READ* : L'utilisateur associé à un Task à partir du modèle `Task`

#### Create une tache

Dans un premier temps je crée une tache :
```js
...
Puis je l'associe à un utilisateur avec la fonction Task.setUser()
