...
Pour débugger notre application il est très utile de pouvoir écrire dans la console du serveur. Habitullement vous utilisez var_dump() et son résultat sera affiché à l'écran. Nous allons redigier le résultat de `var_dump` dans la console du serveur pour simplifier le débuggage.
