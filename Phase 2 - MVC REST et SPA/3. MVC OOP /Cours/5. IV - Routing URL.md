...
3. Dans la classe Router ajoutez un méthode public static nommée `getController()` elle prend en paramètre un string qui est le nom de notre controller.
...
Une route respectant le format suivant :
```
/controller/method/param1/param2/...
```

Vous remarquerez que les différentes parties de l'URI sont séparées par le caractère `/`. Nous allons donc utiliser la fonction `explode()` de php qui permet de séparer une string en plusieurs parties en fonction d'un caractère.

Par exemple :

```php
$uri_elements = explode("/","/product/show/3");
console($uri_elements); // 
```

Le tableau `$uri_elements` contiendra alors les 4 elements suivantes :
```
["","product","show","3"]
```

Donc :
- L'élément 0 est vide car l'uri commence par un `/`
- L'élément 1 est le nom du contrôleur
- L'élément 2 est le nom de la méthode
- L'élément 3 est le premier paramètre

Je peux donc déclarer les variables `$controllerName`, `$methodName` en fonction du tableau `$uri_elements`.
...
Quand au paramètres, je peux utiliser la fonction `array_splice()` pour supprimer les 3 premiers éléments du tableau `$uri_elements` et ne conserver que les paramètres.

```php
$params = array_splice($uri_elements,3); // Je supprime les 3 premiers éléments :
...
Le tableau `$params` contiendra alors :
```
["3"]
```


1. Modifiez la méthode `App::start()` pour récupérer les informations de l'URI en implementant le code ci-dessus :
...
class Person{
    function hello($params){
        echo "Je m' appelle ".$params["name"]."!"; 
    }
}
$person = new Person();
call_user_func([$person,"hello"],["name"=>"Théo"]);
```
> Le premier array contient :
>   - l'objet qui contient la methode (ici `$person`)
>   - et le nom de la méthode (ici `"hello"`).
>
> Le deuxième array contient les paramètres à passer à la fonction via un tableau. (ici `["name"=>"Théo"]`).
