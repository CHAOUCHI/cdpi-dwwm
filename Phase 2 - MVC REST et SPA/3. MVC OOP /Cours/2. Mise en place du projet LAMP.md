...
3. Créez un dossier `src` à l'intérieur de `mvc_project`
4. Créez un fichier `index.php` à l'intérieur de `src`
...
1. Dans le fichier `src/index.php` tapez le code suivant :
...
2. Tapez la commande suivante pour lancer tous les containers :
...
3. Rendez-vous sur `localhost:8080` dans votre navigateur.
