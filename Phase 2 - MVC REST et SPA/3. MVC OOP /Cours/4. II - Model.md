...
> Voici le répo GitHub avec le code final de ce chapitre : 
> https://github.com/CHAOUCHI/mvc_project_soluce/tree/model-soluce
...
0. Avant de commencez assurez vous d'avoir créez une BDD avec une table nommée `Produit` avec quelque produit dedant via phpMyAdmin : http://localhost:8081.
>```mermaid
>erDiagram 
>Produit{
>    id INT
...
### ProductEntity
Une entité represente une ligne de la table. Malgré son nombres de lignes vous verrez que cette classe ne contient en faite que des getter et des setters c'est à dire des méthodes publiques pour définir les attributs de l'objet. 
...
<details>
<summary>Code source complet de ProductEntity</summary>
...
class App{
    public static function start(){
        console(new ProductEntity("Billy",34,"/public/images/billy.png"));
    }
}
```
...
### ProductModel
La création d'un modèle se fait comme suit :
...
2. Ajouter un attribut privé `$getProducts` de type `PDOStatement` pour stocker la requête préparée.
3. Préparer la requête `getProducts` dans le constructeur.
*app/models/ProductModel.php*
```php
class ProductModel{
...
class ProductModel{
    ...
    function __construct()
    {
        ...
        $this->bdd = new PDO("mysql:host=bdd;dbname=boutique","root","root");
        // Création d'une requête préparée qui récupère tout les produits
...
```
Tout les produits de votre BDD devraient s'afficher dans la `console`.

#### Exercices : CRUD ProductModel

Ajoutez les déclarations vierges des méthodes suivantes dans la classe `ProductModel` :
...
class ProductModel{
     ...
     * @return ProductEntity|NULL Un ProductEntity ou NULL si aucune ne correspond à l'$id
    ...
    /**
     * Modifier un produit
     * @return ProductEntity|NULL ProductEntity ou NULL : Le produit modifié après modification ou NULL si l'id n'existe pas.
...
##### Exercice 1 : ProductModel::get()
***Remplissez la méthode `ProductModel::get()`.
Souvenez vous bien de la méthode de travail :***
1. Créez la requête préparée dans le constructeur en ajoutant un attribut privé `$getProduct` de type `PDOStatement`.
...
<details>
<summary>Solution ProductModel Complet </summary>
