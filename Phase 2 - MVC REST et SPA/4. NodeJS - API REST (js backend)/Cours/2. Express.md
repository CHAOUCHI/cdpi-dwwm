...
#### Exercice:
1. Créez une route GET `/lucky/:max` qui renvoi un nombre aléatoire en tre 0 et `:max`
2. Créez une route GET `/lucky/:min/:max` qui renvoi un nombre aléatoire entre `:min` et `:max`.
2. Créez une route GET `/user/:id/tasks` qui renvoi toutes les tâches d'un utilisateur par son id. *Puis testez la dans PostMan*
