# Express - Créer une API REST avec Node.js
## Pré-requis
...
> Il est nécessaire de stopper le serveur MySQL local pour libérer le port 3306 de la machine Linux.

1. Lancer la base de données MySQL via Docker
    a. **Si ce n'est pas déjà fait : créez le conteneur bdd MySQL**
    ```bash
    ...
    b. Ou sinon **lancer le conteneur bdd**
    ```bash
    docker start bdd
    ```
...
```
3. Vous devriez arriver sur le shell MySQL. Exécutez la commande suivante pour vérifier que la base de données app-database a bien été créée.
```sql
...
> Il est obligatoire de coder toute votre application dans la fonction `main()` APRÈS l'appel de la fonction `loadSequelize()`, car cette fonction est asynchrone et :
...
Concrètement, la suite de notre application consistera en la création de routes (URL) sur notre serveur web en utilisant les modèles Sequelize User et Task pour interagir avec la base de données :
- Création d'utilisateur : `User.create(...)`
- Récupération des tâches : `Task.findAll(...)`
- Récupération des tâches d'un utilisateur : `User.findByPk()` et  `user.getTasks()`
...
- Comme tout serveur web, une API REST est composée de routes (URL) auxquelles le client accède via un client HTTP (le plus souvent via l'API `fetch` en JavaScript).
...
Les actions d'un serveur Express sont analogues aux méthodes du protocole HTTP.
...
Une API REST est composée de différentes routes (URL) que l'utilisateur peut appeler via des requêtes HTTP.
...
La méthode get prend deux arguments en paramètre :
- Le chemin de la route : `/hello` (string)
- Une fonction exécutée lorsqu'un utilisateur fait une requête HTTP GET sur cette route. Cette fonction prend deux paramètres :
...
Un serveur HTTP est accessible via une adresse IP (localhost pour une machine locale) et un port (8000 par exemple).

> Définition fonction callback : Une fonction passée en paramètre est appelée une *fonction callback* ou fonction de rappel. 
> Elle est rappelée par le framework Express à chaque fois que l'utilisateur fait une requête HTTP sur la route.

4. Lancez le serveur en local sur le port 8000 avec la fonction `app.listen()`.
...
5. Dans un navigateur, tapez dans la barre d'adresse : `localhost:8000/hello` pour lancer une requête HTTP GET sur la route /hello.

![alt text](image-1.png)

Voilà, vous avez créé votre première route HTTP avec Express !

Cette route renvoie dans le corps de la réponse du texte HTML ; la réponse HTTP brute ressemble à celle-ci.
...
Pour vous entraîner, ajoutez les routes suivantes qui affichent du HTML :
...
Pour effectuer une requête HTTP GET sur la route /hello, il faut :
1. Ouvrir Postman
...
> Plus tard, nous utiliserons Postman pour tester nos routes POST, PUT et DELETE mais également pour ajouter un header d'authentification JWT à nos requêtes.
...
La première route à mettre en place est la route GET /tasks qui, comme son nom et sa méthode l'indiquent, permet de récupérer (GET) toutes les tâches (tasks) de la base de données.
...
> Souvenez-vous qu'en phase de développement front-end nous accéderions à cette route via l'API fetch :
>```js
...
1. Pour vous exercer, créez une route GET `/users` qui renvoie tous les utilisateurs de la table User au format JSON. *Puis testez-la dans Postman*
...
Les paramètres d'URL (path parameters) sont définis dans l'URL avec un `:` avant le nom du paramètre.
...
#### Exercice:
1. Créez une route GET `/lucky/:max` qui renvoie un nombre aléatoire entre 0 et `:max`.
2. Créez une route GET `/lucky/:min/:max` qui renvoie un nombre aléatoire entre `:min` et `:max`.
3. Créez une route GET `/user/:id/tasks` qui renvoie toutes les tâches d'un utilisateur par son id. *Puis testez-la dans Postman*
...
Il nous faut donc :
1. Décoder le body JSON lors de la réception de la requête.
2. Lire le body.
3. L'ajouter dans la table Task.

#### 1. Décoder le body JSON
Pour décoder le body entrant, pas besoin d'utiliser JSON.parse() : Express possède une fonction pour ça :

```js
app.use(express.json())
```
- app.use() est une méthode qui permet de définir une fonction qui s'exécute pour toutes les méthodes HTTP (GET, POST, DELETE, etc.).
- express.json() modifie l'objet request pour nous fournir le body dans l'attribut request.body.
- express.json() est un middleware ! C'est-à-dire une fonction qui s'exécute avant toutes les autres. Donc une fois la fonction exécutée, Express va enchaîner et tester toutes nos autres routes.

Il faut donc placer l'appel à app.use() avant les routes Express :
```js
const app = express();
...
app.post("/task",async (request,response)=>{
    console.log(request.body); // le body devrait apparaître dans la console nodemon
    const newTaskData = request.body; // J'affecte l'objet body à la variable newTaskData
    
    response.json("Route en cours...");
});
```

> ATTENTION : si vous voyez que le body est undefined, c'est que vous avez oublié le middleware `express.json()`.

1. Testez la route POST /task sur Postman en ajoutant le body JSON suivant dans l'onglet Body de la requête : 
```json
{
...
> N'oubliez pas de sélectionner *raw > JSON* comme type de body dans Postman
![alt text](image-6.png)
2. Vérifiez que le body est bien console.log après exécution de la route via Postman
```bash
...
Il suffit maintenant d'appeler la méthode Task.create() pour ajouter une tâche à un utilisateur. Par défaut nous utiliserons l'utilisateur 1 le temps de mettre en place l'authentification sur notre API REST.
...
```
1. Testez la route POST /task sur Postman en ajoutant le body JSON pour ajouter des tâches à l'utilisateur 1.
> N'oubliez pas de sélectionner *raw > JSON* comme type de body dans Postman
![alt text](image-6.png)


3. Ajoutez 4 nouvelles tâches avec la route POST /task et utilisez la route GET /tasks pour vérifier si elles ont bien été ajoutées.
...
Par défaut, Express renvoie le code 200 (OK) pour toutes les routes.

Si une erreur serveur survient (ex : erreur de connexion à la base de données ou erreur SQL), vous devez gérer cette erreur dans un try/catch et renvoyer un code 500 (Internal Server Error) au client.
