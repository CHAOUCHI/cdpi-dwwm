...
> Rendez-vous dans les annexes du cours Sequelize pour voir comme définir plus finement les champs d'une table (taille, unique, not null, default value, gestion d'erreurs).
...
> Si vous vous demandez pourquoi sequeilze vous demande de passer un objet avec un champ where, c'est pour permettre de faire des suppressions plus complexes avec plusieurs conditions (AND, OR, etc).
> Il existent également d'autre champs freres de where utile pour la fonction findAll comme par exemple `default` par exemple qui permet de fournir une valeur par défaut si la table est vide : https://sequelize.org/docs/v6/core-concepts/model-querying-finders/#findorcreate
