...
Sequelize est un ORM (Object-Relational Mapping) pour Node.js qui permet d'accéder à une base de données SQL sans jamais écrire de code SQL. Toutes les actions habituelles du SQL sont accessibles via des objets JavaScript.

> Sequelize fonctionne avec plusieurs SGBD SQL : MySQL, MariaDB, SQLite et PostgreSQL.
...
```
Sequelize s'occupera donc de la rédaction des instructions CREATE TABLE et du placement des clés étrangères.

Quelques points importants à retenir :
- Avec Sequelize, les tables SQL s'appellent des models (modèles) : ils sont créés directement dans le code ; il n'est pas nécessaire de créer un schéma SQL contenant vos CREATE TABLE.
...
const sequelize = new Sequelize("app-database", "root", "root", {
    host: '127.0.0.1',
    dialect: 'mysql' // Définit le SGBD utilisé par Sequelize
});
``` 
...
> Rendez-vous dans les annexes du cours Sequelize pour voir comment définir plus finement les champs d'une table (taille, unique, not null, default value, gestion d'erreurs).
...
Pour effectuer une requête SQL sur un model, il faut récupérer ce model ; il est contenu dans l'attribut `models` de l'objet sequelize :
```js
...
<details>
<summary>Cliquez pour voir le code template complet</summary>
...
### INSERT INTO
Pour effectuer un INSERT INTO, j'utilise la fonction Model.create(), elle prend en paramètre un objet contenant les mêmes champs que ceux déclarés dans la méthode sequelize.define.
...
> Notez l'utilisation de await, c'est la preuve que create effectue une requête SQL sur le réseau (TCP sur le port 3306) : une requête asynchrone donc.
...
Supprimer une ligne d'une table se fait via la fonction `model.destroy()`.

- La fonction destroy prend en paramètre un objet qui lui-même possède un champ `where`.
- Ce champ `where` est lui-même un objet qui contient les conditions.
...
> Si vous vous demandez pourquoi Sequelize vous demande de passer un objet avec un champ where, c'est pour permettre de faire des suppressions plus complexes avec plusieurs conditions (AND, OR, etc).
> Il existe également d'autres champs "frères" de where utiles pour la fonction findAll, par exemple `default` qui permet de fournir une valeur par défaut si la table est vide : https://sequelize.org/docs/v6/core-concepts/model-querying-finders/#findorcreate
...
L'utilisateur peut créer une tâche qui doit lui être associée. Il nous faut donc placer la clé étrangère du UserModel dans une colonne du TaskModel, c'est un OneToMany.
...
Task.belongsTo(User); // liaison Task belongsTo User
// Ensemble ces deux liaisons forment donc une relation OneToMany

await sequelize.sync();
```

Grâce aux fonctions hasMany et belongsTo Sequelize va se charger de placer la clé étrangère userId dans la table de tâches.

> Astuce : c'est toujours le `Model` situé à gauche de la fonction belongsTo qui reçoit la clé étrangère.
...
#### CREATE Créer une tâche associée à un utilisateur
...
Sequelize a créé dynamiquement une nouvelle fonction disponible dans une instance User :
...
Je recommande fortement d'utiliser cette méthode ! D'autant plus qu'une fois l'authentification mise en place, n'importe quelle route de notre serveur aura à sa disposition une variable `user`.

> Cette fonctionnalité avancée de Sequelize s'appelle méthodes mixins ; il en existe plusieurs. Consultez la documentation pour connaître les méthodes disponibles et coder plus rapidement vos applications : https://sequelize.org/docs/v6/core-concepts/assocs/#special-methodsmixins-added-to-instances
...
Dans la même idée que pour la création d'une tâche, Sequelize a créé une méthode mixin pour accéder aux tâches d'un utilisateur à partir d'une instance User.

Pour obtenir toutes les tâches d'un utilisateur, j'utilise la méthode mixin `user.getTasks()` créée automatiquement par Sequelize.
...
#### READ L'utilisateur associé à une Task à partir du modèle `Task`
De la même façon, pour obtenir l'utilisateur associé à une tâche, j'utilise la méthode mixin `task.getUser()` créée automatiquement par Sequelize.
